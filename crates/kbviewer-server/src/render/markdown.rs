...
//! before parsing, using the code-aware scanners in `kbviewer-core`. Standard syntax is left
...
use kbviewer_core::links::{escape_stray_dollars, scan_tags, scan_wikilinks, WikiLink};
use kbviewer_core::model::Heading;
use std::path::Path;
...
fn transclude(
    ...
    let (_, body) = kbviewer_core::frontmatter::split(content);
...
fn tag_document<'a>(
    tag: &str,
    ctx: &'a RenderContext,
) -> Option<&'a kbviewer_core::index::Document> {
...
pub(super) fn enable_task_checkboxes(
    html: &str,
    tasks: &[kbviewer_core::tasks::TaskLine],
) -> String {
...
fn pairing_is_sound(rendered: &[&str], tasks: &[kbviewer_core::tasks::TaskLine]) -> bool {
    use kbviewer_core::tasks::TaskState;
...
mod task_tests {
    ...
    use kbviewer_core::tasks::{TaskLine, TaskState};
