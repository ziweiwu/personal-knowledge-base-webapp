...
use kbviewer_core::index::{Document, Index};
use kbviewer_core::kinds::DocumentKind;
use kbviewer_core::model::DocumentPayload;
...
fn render_docx(
    ...
    let Ok(absolute) = kbviewer_core::paths::resolve_in_root(&root.path, &document.path) else {
        return CachedRender {
            html: String::new(),
            headings: Vec::new(),
    ...
    match std::fs::read(&absolute)
        ...
        .and_then(|bytes| kbviewer_docx::convert(&bytes, &media_base).map_err(|e| e.to_string()))
    {
...
fn render_markdown(root_id: &str, index: &Index, document: &Document) -> CachedRender {
    ...
    let (_, body) = kbviewer_core::frontmatter::split(source);
    ...
    let tasks = kbviewer_core::tasks::task_lines(source);
    CachedRender {
        html: markdown::enhance_images(
...
mod tests {
    use super::*;
    use kbviewer_core::config::RootConfig;
    ...
    fn build_index(
        ...
        let root = std::env::temp_dir().join(format!("kbviewer-render-{label}"));
    ...
    fn headings_are_collected_with_slugs() {
        ...
        let (_, body) = kbviewer_core::frontmatter::split(document.content.as_deref().unwrap());
        let context = RenderContext {
            root_id: "kb",
            path: "a.md",
            index: &index,
