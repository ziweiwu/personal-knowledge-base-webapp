...
use axum::Json;
use kbviewer_core::model::{ApiError, SaveConflict};
...
impl From<kbviewer_core::paths::PathError> for AppError {
    fn from(error: kbviewer_core::paths::PathError) -> Self {
...
mod tests {
    ...
    async fn a_path_escape_is_reported_as_a_plain_not_found() {
        let error: AppError =
            kbviewer_core::paths::PathError::OutsideRoot("../../etc/passwd".into()).into();
