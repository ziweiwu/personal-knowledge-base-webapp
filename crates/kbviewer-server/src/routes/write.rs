...
use axum::Json;
use kbviewer_core::links::{rewrite_wikilinks, RenameContext};
use kbviewer_core::model::{
...
fn toggled_source(
    ...
    use kbviewer_core::tasks::{set_task_state, TaskError, TaskState};
...
fn reject_unless_markdown(state: &AppState, root_id: &str, path: &str) -> AppResult<()> {
    ...
    let document = index
        .get(path)
        .ok_or(AppError::NotFound("document".into()))?;
    if document.kind != kbviewer_core::kinds::DocumentKind::Markdown {
...
fn planned_link_rewrites(
    ...
    let relocated: std::collections::HashMap<&str, &str> = moves
        ...
        .collect();
    let after = kbviewer_core::links::Resolver::new(
        index
            .documents
            .keys()
            .map(|path| {
...
fn moved_documents(
    index: &kbviewer_core::index::Index,
    request: &RenameRequest,
) -> Vec<(String, String)> {
...
mod tests {
    ...
    fn trash_keeps_the_original_folder_layout() {
        let root = std::env::temp_dir().join("kbviewer-trash-layout");
    ...
    fn deleting_the_same_path_twice_does_not_overwrite_the_first() {
        let root = std::env::temp_dir().join("kbviewer-trash-collide");
        let _ = std::fs::remove_dir_all(&root);
    ...
    fn an_atomic_write_replaces_content_completely() {
        let dir = std::env::temp_dir().join("kbviewer-atomic");
