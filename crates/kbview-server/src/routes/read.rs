...
use kbview_core::model::{DocumentPayload, FolderListing, RootInfo, SearchHit, TreeNode};
...
pub async fn search(
    ...
    Ok(Json(kbview_core::search::search(&index, &query.q, limit)))
}
...
) -> AppResult<Json<Vec<kbview_core::model::DocumentMeta>>> {
    let index = state
        .index(&root_id)
        .ok_or(AppError::NotFound("folder".into()))?;

    let wanted = tag.trim_start_matches('#').to_lowercase();
    let mut matches: Vec<kbview_core::model::DocumentMeta> = index
        .documents
        .values()
        .filter(|document| {
            document
