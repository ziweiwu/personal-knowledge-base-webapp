...
mod tests {
    ...
    fn store(label: &str) -> AuthStore {
        let dir = std::env::temp_dir().join(format!("kbview-auth-{label}"));
    ...
    fn users_and_sessions_survive_a_restart() {
        let dir = std::env::temp_dir().join("kbview-auth-persist");
        let _ = std::fs::remove_dir_all(&dir);
    ...
    fn an_expired_session_is_not_accepted() {
        let dir = std::env::temp_dir().join("kbview-auth-expired");
        let _ = std::fs::remove_dir_all(&dir);
    ...
    fn the_user_file_is_not_world_readable() {
        ...
        store.add_user("a@b.c", "pw").unwrap();
        let dir = std::env::temp_dir().join("kbview-auth-perms");
