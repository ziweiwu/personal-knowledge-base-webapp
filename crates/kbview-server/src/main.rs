...
use kbview_server::{cli, router, state, watch};
use state::AppState;
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "kbview=info,tower_http=warn".into()),
        )
        .init();
}
...
async fn serve(app: axum::Router, address: SocketAddr) -> Result<()> {
    ...
    tracing::info!(%address, "kbview listening");
    eprintln!("kbview listening on http://{address}");
