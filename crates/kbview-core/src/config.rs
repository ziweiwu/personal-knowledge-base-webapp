...
pub enum ConfigError {
    #[error("no config at {0}: copy kbview.config.example.json and set your folder path")]
    Missing(PathBuf),
    #[error("{0}: {1}")]
...
impl Config {
    ...
    /// `KBVIEW_ROOT_<ID>` repoints a root without editing the file, so the same config
    /// works in a container where the folder is mounted somewhere else.
    fn apply_env_overrides(&mut self) {
        for root in &mut self.roots {
            let key = format!("KBVIEW_ROOT_{}", root.id.to_uppercase().replace('-', "_"));
...
mod tests {
    ...
    fn write_config(label: &str, body: &str) -> PathBuf {
        let path = std::env::temp_dir().join(format!("kbview-config-{label}.json"));
    ...
    fn loads_a_minimal_config_and_defaults_the_rest() {
        let path = write_config(
            "minimal",
            r#"{"roots":[{"id":"kb","path":"/tmp/kbview-test-vault"}]}"#,
        );
    ...
    fn an_env_override_is_canonicalised_like_a_configured_path() {
        let real = std::env::temp_dir().join("kbview-config-envroot");
        ...
        unsafe { std::env::set_var("KBVIEW_ROOT_ENVROOT", &real) };
        let config = Config::load(&path);
        unsafe { std::env::remove_var("KBVIEW_ROOT_ENVROOT") };
    ...
    fn a_root_reached_through_a_symlink_is_stored_canonically() {
        let real = std::env::temp_dir().join("kbview-config-real");
        let _ = std::fs::remove_dir_all(&real);
        std::fs::create_dir_all(&real).unwrap();
