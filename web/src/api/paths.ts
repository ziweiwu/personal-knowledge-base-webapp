...
/** What delete moved aside for one collection, with a Restore per entry. */
export function trashRoute(rootId: string): string {
  return `/trash/${encodeURIComponent(rootId)}`;
}
