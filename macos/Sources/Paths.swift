...
enum Paths {
    static let bundleIdentifier = "com.ziweiwu.kbview"
    ...
    static let accountEmail = "kbview-app@localhost"
    ...
    private static var supportOverride: String? {
        ProcessInfo.processInfo.environment["KBVIEW_APP_SUPPORT"]
    }
    ...
    static var supportDirectory: URL {
        ...
        return FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask)[0]
            .appendingPathComponent("KBView", isDirectory: true)
    }

    static var configFile: URL {
        supportDirectory.appendingPathComponent("kbview.config.json")
    }
    ...
    /// The bundled server. Named `kbview-server` rather than `kbview` because the
    /// wrapper's own executable is `KBView`, and the Mac's filesystem is case
    /// insensitive: `kbview` and `KBView` are one file, so copying the server in beside
    ...
    static let serverBinaryName = "kbview-server"
